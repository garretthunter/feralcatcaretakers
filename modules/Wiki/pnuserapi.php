<?php
// $Id: pnuserapi.php,v 1.3 2002/11/12 13:08:25 magicx Exp $
// ----------------------------------------------------------------------
...
function PNwikiInclude($retour) { 
    ...
    $retour = "<table border=0 cellpadding=8 cellspacing=1 width=100%><tr><td align=left><font class=\"pn-normal\">" . $retour . "</td></tr></table>";
    return $retour;         
}
...
function wiki_userapitransform($cContent)
{
    ...
    static $PNlWithHtml;
    if (!isset($PNWithHtml)) {
        $PNWithHtml = pnModGetVar('Wiki', 'WithHTML');
    }
    static $PNAllowedProtocols;
    if (!isset($PNAllowedProtocols)) {
        $PNAllowedProtocols = pnModGetVar('Wiki', 'AllowedProtocols');
